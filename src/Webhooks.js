...
function verifyStripeSignature(payload, sigHeader, secret) {
  if (!secret || secret.startsWith('YOUR_')) {
    Logger.log('WARNING: Stripe webhook secret not set -- skipping verification');
    return true;
  }
  try {
    const parts = {};
    ...
    if (Math.abs(now - parseInt(timestamp)) > 300) {
      Logger.log('Stripe webhook timestamp too old');
      return false;
    }
    const expectedSig = computeHmacSha256(secret, timestamp + '.' + payload);
